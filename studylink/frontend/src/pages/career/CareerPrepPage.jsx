...
export default function CareerPrepPage(){
  ...
  const [loading,setLoading]=useState(true); const [tool,setTool]=useState(null); const [query,setQuery]=useState('');
  ...
  const filteredResources=useMemo(()=>data.resources.filter(r=>`${r.title} ${r.description||''}`.toLowerCase().includes(query.toLowerCase())),[data.resources,query]);
  const upcoming=data.sessions.find(s=>s.status==='scheduled' && (!s.start_time || new Date(s.start_time)>new Date()));
  const currentQuestions=tool==='hr'?data.questions.filter(q=>q.category==='hr'):tool==='mock'?data.questions:data.questions.filter(q=>q.category!=='technical');
  const q=currentQuestions[questionIndex%Math.max(1,currentQuestions.length)];

  async function saveAttempt(){ if(!q) return; try{await api.saveCareerPractice({question_id:q.id,answer_text:answer,confidence},token);notify('Réponse enregistrée');setAnswer('');setQuestionIndex(i=>i+1);load()}catch(e){notify(e.message)}}
  ...
  function openAction(key){ if(key==='workshop') return navigate('/bootcamps'); setTool(key); }
  ...
  return <AppShell><div className="career-page-v2"><PageHeader title="Carrière & emploi" subtitle="Préparez vos candidatures, entraînez-vous et suivez vos progrès."/>
    <section className="career-overview-grid">
      <div className="career-goal-card"><div className="career-card-head"><div><span className="eyebrow">Votre objectif</span><h2>{data.goals?.target_role||'Définissez votre prochain poste'}</h2></div><Target size={34}/></div>
        ...
        <button className="career-link-btn" onClick={()=>setTool('goals')}>Mettre à jour mon objectif</button></div>
    ...
    <div className="career-resource-grid">{filteredResources.map(r=><button key={r.id} className="career-resource-card" onClick={()=>{if(r.url?.startsWith('/career-prep'))setTool(resourceTool(r));else if(r.url)window.open(r.url,'_blank')}}><ResourceIcon type={r.resource_type}/><div><b>{r.title}</b><p>{r.description}</p><small>{resourceLabel(r.resource_type)} · Gratuit</small></div></button>)}</div>

    {tool&&<div className="career-modal-backdrop" onClick={()=>setTool(null)}><div className="career-modal" onClick={e=>e.stopPropagation()}><button className="career-modal-close" onClick={()=>setTool(null)}><X/></button>{tool==='goals'?<GoalEditor goals={goals} setGoals={setGoals} save={saveGoals}/>:tool==='cv'?<CvTool submissions={data.cv_submissions} notify={notify}/>:tool==='feedback'?<FeedbackTool data={data}/>:tool==='questions'?<QuestionLibrary questions={data.questions}/>:<PracticeTool title={tool==='hr'?'Simulation RH':tool==='pitch'?'Pitch en 2 minutes':tool==='checklist'?'Checklist entretien':"Simulation d’entretien"} question={q} answer={answer} setAnswer={setAnswer} confidence={confidence} setConfidence={setConfidence} save={saveAttempt} next={()=>{setAnswer('');setQuestionIndex(i=>i+1)}}/>}</div></div>}
  </div></AppShell>
}
...
function resourceTool(r){if(r.url?.includes('cv'))return'cv';if(r.url?.includes('checklist'))return'checklist';if(r.url?.includes('pitch'))return'pitch';return'questions'}
