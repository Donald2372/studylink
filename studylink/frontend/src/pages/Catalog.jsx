...
// Petites icônes SVG par clé de catégorie (couleurs gérées inline)
function CategoryIcon({ icon }) {
  ...
  switch (icon) {
    ...
    case 'sigma':
      return <svg {...common}><path d="M18 7V5H6l6 7-6 7h12v-2" /></svg>;
    case 'atom':
      return <svg {...common}><circle cx="12" cy="12" r="1" /><ellipse cx="12" cy="12" rx="10" ry="4.5" /><ellipse cx="12" cy="12" rx="10" ry="4.5" transform="rotate(60 12 12)" /><ellipse cx="12" cy="12" rx="10" ry="4.5" transform="rotate(120 12 12)" /></svg>;
    case 'globe':
      return <svg {...common}><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15 15 0 0 1 0 20 15 15 0 0 1 0-20" /></svg>;
    case 'briefcase':
      return <svg {...common}><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /></svg>;
    case 'brain':
      return <svg {...common}><path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.96.44 2.5 2.5 0 0 1-2.96-3.08 3 3 0 0 1-.34-5.58 2.5 2.5 0 0 1 1.32-4.24 2.5 2.5 0 0 1 4.44-1.04Z" /></svg>;
    case 'network':
      return <svg {...common}><circle cx="12" cy="5" r="2" /><circle cx="5" cy="19" r="2" /><circle cx="19" cy="19" r="2" /><path d="M12 7v6m0 0-5 4m5-4 5 4" /></svg>;
    case 'chart':
      return <svg {...common}><path d="M3 3v18h18M7 16v-5M12 16V8M17 16v-3" /></svg>;
    default:
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="mt-6 flex items-center gap-2 rounded-xl2 border border-black/10 bg-white px-4 py-3 shadow-card">
        <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" className="text-ink/40">
      ...
      {/* Filtres (chips visuels) */}
      <div className="mt-4 flex flex-wrap gap-2">
        {['Catégories', 'Niveau', 'Type', 'Langue'].map((f) => (
          <button
            key={f}
            className="flex items-center gap-1 rounded-lg border border-black/10 bg-white px-3 py-2 text-sm font-medium text-ink/70 transition hover:bg-black/5"
          >
            {f}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {categories.map((cat) => (
              <Link
                ...
                className="flex flex-col items-center rounded-xl2 border border-black/5 bg-white p-5 text-center shadow-card transition hover:shadow-pop"
              >
                <span
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {popular.map((c) => (
              <Link
                key={c.id}
                to={`/course/${c.id}`}
                className="overflow-hidden rounded-xl2 border border-black/5 bg-white shadow-card transition hover:shadow-pop"
              >
                <div
          ...
          {/* Tutoriels rapides (statiques, fidèles à la maquette) */}
          <div className="mt-10 flex items-center justify-between">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[
              ['Installer Python', '8 min'],
              ['Git & GitHub', '12 min'],
              ['Docker', '15 min'],
            ].map(([title, dur]) => (
              <div
                key={title}
                className="flex items-center justify-between rounded-xl2 border border-black/5 bg-white p-4 shadow-card"
              >
                <div>
