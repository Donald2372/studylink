...
export default function CourseModulesPage(){
  ...
  const fallback=id===PYTHON_COURSE_SLUG||id==='demo-python'||isEnglishCourseId(id);
  useEffect(()=>{api.getCourse(id).then(r=>{setData(r);setOpen(r.modules?.map((_,i)=>i) || [])}).catch(e=>{if(fallback){const courseData=isEnglishCourseId(id)?englishCourseFallback:pythonCourseFallback;setData(courseData);setOpen(courseData.modules.map((_,i)=>i))}else setError(e.message)})},[id,fallback]);
